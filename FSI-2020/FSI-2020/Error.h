...
#define ERROR_MAXSIZE_MESSAGE 200									//Максимальная длина сообщения рб ошибке
#define ERROR_ENTRY_NODEF(id)		ERROR_ENTRY(-id, "Неопределенная ошибка") //1 неопределённый элемент таблицы

// 10 неопределённых элементов таблицы ошибок
#define ERROR_ENTRY_NODEF10(id)		ERROR_ENTRY_NODEF(id+0),ERROR_ENTRY_NODEF(id+1),ERROR_ENTRY_NODEF(id+2),ERROR_ENTRY_NODEF(id+3),\
									ERROR_ENTRY_NODEF(id+4),ERROR_ENTRY_NODEF(id+5),ERROR_ENTRY_NODEF(id+6),ERROR_ENTRY_NODEF(id+7),\
									ERROR_ENTRY_NODEF(id+8),ERROR_ENTRY_NODEF(id+9)

// 100 неопределённых элементов таблицы ошибок
...
#define ERROR_MAX_ENTRY	1000 //Максимальное количество элементов в таблице ошибок
...
namespace Error
{
	struct ERROR //тип исключения для throw ERROR_THROW | ERROR_THROW_IN и catch(ERROR)
	{
		...
		{
			short line;							//номер строки(0...)
			short col;							//номер позиции в строки(0...)
		} inext;
	};
